...
def gross_for_director(nds, row_counter)
  num_movies = 0
  total = 0
  while num_movies < nds[row_counter][:movies].length do
    total += nds[row_counter][:movies][num_movies][:worldwide_gross]
    num_movies += 1
  end
  total
end
  ...
  def directors_totals(nds)
    ...
    row_counter = 0
      while row_counter < nds.length do
       ...
       director_total = gross_for_director(nds, row_counter)
